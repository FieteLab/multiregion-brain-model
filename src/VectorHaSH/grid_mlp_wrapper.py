...
class GridNew2dWrapper(BaseWrapper):
    ...
    def init_gcpc(self, arg_gcpc, grid_info):
        ...
        self.grid_info = [Wggs, Wgp, Wpg, Wsp, Wps, module_gbooks, module_sizes, g, sbook_obs, pbook_obs, theta_sp, theta_ps, self.mask_pg, self.mask_ps, self.W_L, self.W_R, 0]
    ...
    def reset(self, episode_idx=0):
        ...
        # p = torch.relu(self.mask_pg*p_g) + torch.relu(self.mask_ps*p_s)
        # p = torch.relu(self.mask_pg*p_g+self.mask_ps*p_s)
        if self.new_model:
    ...
    def grid_step(self, sinit, action, same_as_prev_pos, force_prev=False, recon_only=False):
        ...
        Wggs, Wgp, Wpg, Wsp, Wps, module_gbooks, module_sizes, g, sbook_obs, pbook_obs, theta_sp, theta_ps, mask_pg, mask_ps, W_L, W_R, p_s = self.grid_info
        ...
        if recon_only:
            ...
            p, g, s, sbook_obs, pbook_obs, Wsp, Wps, theta_sp, theta_ps, p_s = run_grid_cells_2d_new_model(sinit, disp, Wggs, Wgp, Wpg, 
                                                                                     Wsp, Wps, module_gbooks, module_sizes, g,
            ...
            if all(x == 'both' for x in self.grid_assignment) and not same_as_prev_pos:
                ...
                """
                assert 'check behavior in new model before calling'
            ...
            if not all(x == 'both' for x in self.grid_assignment) and 'both' in self.grid_assignment and not same_as_prev_pos:
                ...
                p, g, s, sbook_obs, pbook_obs, Wsp, Wps, theta_sp, theta_ps = run_grid_cells_2d(sinit, 'trigger', Wggs, Wgp, Wpg, 
                                                                                     ...
                                                                                     verbose=self.verbose, allocated_grids=self.allocated_grids, 
                                                                                     gap=gap)
            self.grid_info = [Wggs, Wgp, Wpg, Wsp, Wps, module_gbooks, module_sizes, g, sbook_obs, pbook_obs, theta_sp, theta_ps, mask_pg, mask_ps, W_L, W_R, p_s]
