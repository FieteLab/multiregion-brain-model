...
class Grid2dRecurWrapper(BaseWrapper):
    ...
    def init_gcpc(self, arg_gcpc, grid_info):
        ...
        self.grid_info = [Wggs, Wgp, Wpg, Wsp, Wps, module_gbooks, module_sizes, g, sbook_obs, pbook_obs, theta_sp, theta_ps, Wpp, theta_pp]
    ...
    def reset(self, episode_idx=0):
        ...
        # breakpoint()
        p = torch.relu(self.grid_info[2]@self.grid_info[7]) # reset has no recurrence kicks in.
    ...
    def grid_step(self, sinit, action, same_as_prev_pos, force_prev=False, recon_only=False):
        ...
        Wggs, Wgp, Wpg, Wsp, Wps, module_gbooks, module_sizes, g, sbook_obs, pbook_obs, theta_sp, theta_ps, Wpp, theta_pp = self.grid_info
        ...
        if recon_only:
            ...
            condition_1 = all(x == 'both' for x in self.grid_assignment) and not same_as_prev_pos
            condition_2 =  not all(x == 'both' for x in self.grid_assignment) and 'both' in self.grid_assignment and not same_as_prev_pos
            if not self.is_eval: # at training
                ...
                this_eval = self.is_eval
            p, g, s, sbook_obs, pbook_obs, Wsp, Wps, theta_sp, theta_ps, Wpp, theta_pp = run_grid_cells_2d(sinit, disp, Wggs, Wgp, Wpg, 
                                                                                     Wsp, Wps, module_gbooks, module_sizes, g,
                                                                                     sbook_obs, pbook_obs, theta_sp, theta_ps, 
                                                                                     self.use_cuda, this_eval, force_prev, # note I set is_eval according to above condition
            ...
            if all(x == 'both' for x in self.grid_assignment) and not same_as_prev_pos:
                ...
                """
                p, g, s, sbook_obs, pbook_obs, Wsp, Wps, theta_sp, theta_ps, Wpp, theta_pp = run_grid_cells_2d(sinit, (0,1), Wggs, Wgp, Wpg, 
                                                                                     Wsp, Wps, module_gbooks, module_sizes, g,
                                                                                     sbook_obs, pbook_obs, theta_sp, theta_ps, 
            ...
            if not all(x == 'both' for x in self.grid_assignment) and 'both' in self.grid_assignment and not same_as_prev_pos:
                p, g, s, sbook_obs, pbook_obs, Wsp, Wps, theta_sp, theta_ps, Wpp, theta_pp = run_grid_cells_2d(sinit, 'trigger', Wggs, Wgp, Wpg, 
                                                                                     Wsp, Wps, module_gbooks, module_sizes, g,
                                                                                     sbook_obs, pbook_obs, theta_sp, theta_ps, 
                                                                                     self.use_cuda, self.is_eval, force_prev, 
                                                                                     verbose=self.verbose, allocated_grids=self.allocated_grids, 
